...
from src.utils.network_define_maskrcnnresnet50 import \
...
def load_pretrained_ckpt(net, load_path, device_target):
    ...
    if config.pretrain_epoch_size == 0:
        for item in list(param_dict.keys()):
            if not (item.startswith('backbone') or
                    item.startswith('rcnn_mask')):
                param_dict.pop(item)
...
def train_maskrcnn():
    ...
    mindrecord_file = config.data_root
    ...
    # set loss function, learning rate and optimizer.设
    loss = LossNet()
    lr = Tensor(dynamic_lr(config, rank_size=device_num,
                           start_steps=config.pretrain_epoch_size *
                           dataset_size),
                mstype.float32)
    ...
    if config.save_checkpoint:
        ...
        ckptconfig = \
            CheckpointConfig(save_checkpoint_steps=ckpt_step,
                             keep_checkpoint_max=config.keep_checkpoint_max)
        save_checkpoint_path = os.path.join(config.save_checkpoint_path,
                                            'ckpt_' + str(rank) + '/')
        # apply saved weights.
        ckpoint_cb = ModelCheckpoint(prefix='mask_rcnn',
